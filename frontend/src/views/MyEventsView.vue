<template>
  <div class="pb-20" :style="{ background: 'var(--tg-secondary)', minHeight: '100dvh' }">
    <div class="tg-section-header pt-4">{{ $t('myEvents.title') }}</div>
    ...
    <!-- Empty -->
    <div v-else-if="eventsStore.myEvents.length === 0" class="tg-section">
      <div class="py-12 text-center">
        <p class="text-[15px] font-medium" :style="{ color: 'var(--tg-hint)' }">{{ $t('myEvents.empty') }}</p>
        <p class="text-[13px] mt-1" :style="{ color: 'var(--tg-hint)' }">{{ $t('myEvents.createFirst') }}</p>
    ...
    <div v-else class="tg-section">
      <template v-for="(event, i) in eventsStore.myEvents" :key="event.id">
...
onMounted(() => { if (userStore.isLoggedIn) eventsStore.fetchMyEvents() })
</script>
